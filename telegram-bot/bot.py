...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"\U0001f4c3 Ваш UserID: tg#{user_id}\n\n"
        "Чтобы привязать аккаунт,\n"
        "Введите в игровой чат команду:\n"
        "/link\n\n"
        "\U0001f4c3 Список команд бота:\n"
        "\u25aa\ufe0f /link <код> - Привязать аккаунт\n"
        "\u25aa\ufe0f /unlink - Отвязать аккаунт\n"
        "\u25aa\ufe0f /status - Информация об аккаунте\n"
        "\u25aa\ufe0f /tfa - Двухэтапная авторизация\n"
        "\u25aa\ufe0f /restore - Получить новый пароль\n"
        "\u25aa\ufe0f /kick - Кикнуть аккаунт\n"
        "\u25aa\ufe0f /ban - Заблокировать аккаунт"
    )
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"\U0001f4ca Информация об аккаунте:\n\n"
        f"\u25aa\ufe0f Ник: {info['username']}\n"
        f"\u25aa\ufe0f Статус: {'\U0001f7e2 В сети' if info.get('online') else '\U0001f534 Не в сети'}\n"
        f"\u25aa\ufe0f Блокировка: {'✅ Да' if info.get('banned') else '❌ Нет'}\n"
        f"\u25aa\ufe0f 2FA: {'✅ Включена' if info.get('tfa_enabled') else '❌ Выключена'}\n"
        f"\u25aa\ufe0f Последний IP: {info.get('last_ip', '—')}\n"
        f"\u25aa\ufe0f Последний вход: {info.get('last_city', '—')}, {info.get('last_country', '—')}"
    )
    keyboard = []
    if info.get("banned"):
        keyboard.append([InlineKeyboardButton("✅ Разблокировать", callback_data="unban")])
    if info.get("online"):
        keyboard.append([InlineKeyboardButton("\U0001f4a2 Кикнуть", callback_data="kick")])
    else:
        keyboard.append([InlineKeyboardButton("\U0001f511 Восстановить пароль", callback_data="restore")])
...
async def restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"\U0001f511 Ваш новый пароль: {new_password}\n\n"
